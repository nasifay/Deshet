...
export async function GET(request: NextRequest) {
  try {
    ...
    const groupsWithIds = targetGroups.map((group: any, index: number) => ({
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: settings.targetGroups[settings.targetGroups.length - 1] });
  } catch (error: any) {
