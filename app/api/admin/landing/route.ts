...
export async function GET(request: NextRequest) {
  try {
    ...
    const landingPage = await Page.findOne({ slug: 'landing' }).populate('author', 'name email');
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (landingPage) {
      ...
      landingPage = await Page.findById(landingPage._id).populate('author', 'name email');
    }
