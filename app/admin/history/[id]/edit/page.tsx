...
export default function EditHistoryPage() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (data.success) {
        setFormData({
          title: data.data.title || "HISTORY",
          subtitle: data.data.subtitle || "",
          heroImages: urlsToPendingImages(data.data.heroImages || []),
          introductionParagraphs: data.data.introductionParagraphs?.length
            ? data.data.introductionParagraphs
            : [""],
          milestonesImage: urlToPendingImage(data.data.milestonesImage || ""),
          timelineSections: data.data.timelineSections || [],
          closingQuote: data.data.closingQuote || "",
          status: data.data.status || "draft",
        });
      } else {
        alert(data.error || "Failed to fetch history entry");
        router.push("/admin/history");
      }
    } catch (error) {
      console.error("Error fetching history:", error);
      alert("Failed to fetch history entry");
      router.push("/admin/history");
    } finally {
      setFetching(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || "Failed to update history entry");
      }
    } catch (error) {
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              Update your history page content
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end space-x-4">
          ...
          >
            ...
            <span>{loading ? "Updating..." : "Update History Entry"}</span>
          </button>
        </div>
      </form>
    </div>
