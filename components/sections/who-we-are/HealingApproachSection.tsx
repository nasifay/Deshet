...
interface HealingApproachSectionProps {
  title?: string | { en: string; am: string };
  content?: string | { en: string; am: string };
  image?: string;
}

export function HealingApproachSection({
  title,
  content,
  image,
}: HealingApproachSectionProps) {
  const { locale } = useTranslation();

  const titleText = getBilingualText(
    title,
    locale,
    locale === "am" ? "የምናደርገው የፈውስ አቀራረብ" : "Our Healing Approach"
  );
